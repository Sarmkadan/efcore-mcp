...
public sealed class MigrationInspector(IDbContextProvider contextProvider) : IMigrationInspector
{
    ...
    public ModelDiff DiffAgainstSnapshot()
    {
        ...
        var differ = GetRequiredDesignTimeService<IMigrationsModelDiffer>(services);
        var designTimeModel = GetRequiredDesignTimeService<IDesignTimeModel>(services).Model;
    ...
    private static T GetRequiredDesignTimeService<T>(IServiceProvider services) where T : notnull
    {
        ArgumentNullException.ThrowIfNull(services);
        ...
        catch (InvalidOperationException exception)
        {
            throw new InvalidOperationException(
                $"Unable to resolve the required design-time service '{typeof(T).FullName}'. " +
                "Ensure the context assembly references Microsoft.EntityFrameworkCore.Design and configures EF Core design-time services.",
                exception);
        }
    }
}
