...
return new QueryRejection(QueryRejectionCode.EmptyQuery, "Query is empty.");
...
return new QueryRejection(QueryRejectionCode.MultipleStatements, "Multiple statements are not allowed.");
...
return new QueryRejection(QueryRejectionCode.EmptyStatement, "Statement is empty after normalization.");
...
return new QueryRejection(QueryRejectionCode.ForbiddenKeyword, $"Keyword '{keyword}' is not allowed in read-only mode.");
}
}
...
return new QueryRejection(QueryRejectionCode.ForbiddenKeyword, "Keyword 'into' is not allowed in read-only mode.");
...
return new QueryRejection(QueryRejectionCode.NotSelect, "Only SELECT (or WITH ... SELECT) queries are allowed.");
...
return new QueryRejection(QueryRejectionCode.CteMissingSelect, "WITH clause must be followed by a SELECT statement.");
...
{
var keyword = match.Groups[1].Value.ToLowerInvariant();
return new QueryRejection(QueryRejectionCode.WriteOperationInCte, $"Write operation '{keyword}' in CTE definition is not allowed in read-only mode.");
}
}
